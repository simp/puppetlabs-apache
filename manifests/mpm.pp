define puppetlabs_apache::mpm (
  ...
  case $::osfamily {
    'debian': {
      file { "${::puppetlabs_apache::mod_enable_dir}/${mpm}.conf":
        ensure  => link,
        target  => "${::puppetlabs_apache::mod_dir}/${mpm}.conf",
        require => Exec["mkdir ${::puppetlabs_apache::mod_enable_dir}"],
        before  => File[$::puppetlabs_apache::mod_enable_dir],
        notify  => Service['httpd'],
      }

      if versioncmp($apache_version, '2.4') >= 0 {
        file { "${::puppetlabs_apache::mod_enable_dir}/${mpm}.load":
          ensure  => link,
          target  => "${::puppetlabs_apache::mod_dir}/${mpm}.load",
          require => Exec["mkdir ${::puppetlabs_apache::mod_enable_dir}"],
          before  => File[$::puppetlabs_apache::mod_enable_dir],
          notify  => Service['httpd'],
        }
